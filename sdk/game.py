...
class Game:
    ...
    def force_loss(self, type='timeout'):
        ...
        winner_s = self.get_player_by_color(win_color)
        loser_s = self.get_player_by_color(loss_color)
        error_map = {
            'timeout': f'{loser_s} think more than {self._move_timeout}s, {winner_s} wins',
            'legal': f'{loser_s} violate move rules, {winner_s} wins',
            'board': f'{loser_s} modifies the board, {winner_s} wins',
            'exception': f'{loser_s} error due to exception, {winner_s} wins'
        }
        error = error_map[type] or ""
        diff = 64
    ...
    def run(self, timeout=60):
        ...
        while True:
            ...
            try:
                for i in range(0, 3):
                    # 获取落子位置
                    timeout = min(self._cum_timeout - total_time[color], self._move_timeout)
                    action = func_timeout(
                        timeout,
